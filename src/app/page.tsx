...
export default function CRMApp() {
  ...
  async function agregarTarea(e: React.FormEvent) {
    ...
    if (data) {
      setTareasCliente([...tareasCliente, data[0]]);
      setNuevaTareaTitulo("");
      setNuevaTareaFecha("");
      fetchTodasTareas();
    }
  ...
  async function toggleTarea(tareaId: string, actual: boolean) {
    ...
    setTareasCliente(tareasCliente.map((t) => (t.id === tareaId ? { ...t, completada: !actual } : t)));
    fetchTodasTareas();
  }

  async function eliminarTarea(tareaId: string) {
    await supabase.from("tareas").delete().eq("id", tareaId);
    setTareasCliente(tareasCliente.filter((t) => t.id !== tareaId));
    fetchTodasTareas();
  }
    ...
    if (tipoPago === "unico") {
      ...
      for (let i = 0; i < n; i++) {
        ...
        arr.push({
          ...
          estado: "pendiente",
          metodo_pago: metodoPago,
          notas: `Cuota ${i + 1}/${n}${notaPago ? " - " + notaPago : ""}`,
        });
      }
    }
  ...
  async function cargarSaldos() {
    ...
    try {
      ...
      setBalances({ error: "No se pudo cargar: " + e.message });
    }
    setLoadingBal(false);
  }
  ...
  const totalConvertidos = clientesNoSpam.filter((c) =>
    ["pago_recibido", "trabajo_proceso", "trabajo_completado"].includes(c.estado)
  ).length;
    ...
    <div className="flex flex-col md:flex-row h-[100dvh] w-screen bg-background text-gray-200 overflow-hidden font-sans">
      ...
      <aside className="fixed bottom-0 w-full h-16 bg-surface border-t border-border flex flex-row items-center justify-around z-40 md:relative md:w-20 md:h-full md:border-r md:border-t-0 md:flex-col md:py-6 md:justify-between">
        ...
        <nav className="flex flex-row md:flex-col gap-1 md:gap-3 w-full justify-around md:px-3">
          {menuItems.map((item) => (
            <button
              ...
              className={`p-2 md:p-3.5 rounded-xl flex flex-col items-center gap-1 transition-all flex-1 md:flex-none ${
                tab === item.id
                  ? "text-purple-400 md:bg-purple-600 md:text-white"
        ...
        <div className="hidden md:flex flex-col items-center gap-3">
          <button onClick={() => setShowAdmin(true)} className="text-gray-500 hover:text-purple-300 p-1" title="Admin">
              ...
              <div className="p-4 border-b border-border flex flex-col gap-3 pt-6 md:pt-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button className="md:hidden text-gray-500" onClick={() => setShowAdmin(true)}>
                      <Shield className="w-4 h-4" />
                    </button>
                  </div>
                </div>
                      ...
                      className={`flex-1 py-1.5 rounded-md transition-all capitalize ${
                        filtroCanal === f
                          ? f === "spam"
                            ...
                            : "bg-surfaceHover text-white font-medium"
                          : "text-gray-500"
                      }`}
                    >
                  ...
                  conversacionesFiltradas.map((conv) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full p-4 flex items-start gap-3 text-left hover:bg-surfaceHover transition-colors ${
                          selectedConv?.id === conv.id ? "bg-surfaceHover border-l-4 border-purple-500" : ""
                        }`}
                      >
                        <div className="relative flex-shrink-0">
                          <div className="w-12 h-12 rounded-full bg-surface border border-border flex items-center justify-center text-purple-400 font-bold overflow-hidden">
                            {cliente?.foto_url ? (
                              ...
                              <span>{displayName?.charAt(0) || "W"}</span>
                            )}
                          </div>
              ...
              <div className="flex-1 flex w-full h-full absolute inset-0 md:relative bg-background z-20">
                ...
                <section className={`flex-1 flex flex-col h-full ${showMobileDetails ? "hidden md:flex" : "flex"}`}>
                  ...
                  <header className="h-16 px-4 md:px-6 border-b border-border bg-surface/80 backdrop-blur-md flex items-center justify-between flex-shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setShowMobileDetails(true)} className="md:hidden p-2 text-gray-400">
                        ...
                        <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                          ...
                          >
                            {msg.tipo_contenido === "audio" && msg.url_archivo ? (
                              <audio controls src={msg.url_archivo} className="max-w-[220px] h-10" />
                            ...
                            <span className={`block text-[9px] mt-1 ${isMe ? "text-purple-200 text-right" : "text-gray-500"}`}>
                  ...
                  <div className="p-3 border-t border-border bg-surface/80 backdrop-blur-md flex items-center gap-2 flex-shrink-0">
                    ...
                    <button
                      ...
                      disabled={clienteActual.es_spam || isRecording}
                      className="p-2.5 text-gray-400 hover:text-purple-400 hover:bg-surfaceHover rounded-full transition-colors disabled:opacity-40"
                    >
                    ...
                    {isRecording ? (
                      <div className="flex-1 bg-red-950/30 border border-red-900/50 rounded-full px-4 py-2 flex items-center justify-between">
                        <div className="flex items-center gap-2 text-red-400 text-sm font-medium">
                          ...
                          <Mic className="w-4 h-4" /> {formatTime(recordingTime)}
                        </div>
                        <div className="flex items-center gap-1">
                          <button onClick={cancelRecording} className="p-1.5 text-gray-400 hover:text-white rounded-full">
                            <Trash2 className="w-4 h-4" />
                          </button>
                          <button onClick={stopRecording} className="p-1.5 text-white bg-red-600 hover:bg-red-500 rounded-full shadow-lg">
                      ...
                      <form onSubmit={handleSendMessage} className="flex-1 flex items-center gap-2">
                        <input
                          ...
                          disabled={clienteActual.es_spam}
                          className="flex-1 bg-background border border-border rounded-full px-4 py-2.5 text-sm text-gray-100 placeholder-gray-500 focus:outline-none focus:border-purple-500 disabled:opacity-50"
                        />
                        {nuevoMensaje.trim() ? (
                          ...
                          <button
                            ...
                            disabled={clienteActual.es_spam}
                            className="bg-surface border border-border text-purple-400 hover:bg-purple-600 hover:text-white hover:border-purple-600 p-2.5 rounded-full transition-colors disabled:opacity-50"
                          >
                ...
                >
                  <header className="md:hidden flex items-center p-4 border-b border-border bg-background sticky top-0 z-10">
                    <button onClick={() => setShowMobileDetails(false)} className="p-2 -ml-2 text-gray-400">
                      <ArrowLeft className="w-5 h-5" />
                    </button>
                  ...
                  <div className="p-5 space-y-5">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={toggleSpam}
                        className={`flex-1 flex justify-center items-center gap-1.5 py-2 rounded-lg border text-xs font-medium transition-all ${
                          clienteActual.es_spam
                            ? "bg-red-950 border-red-700 text-red-400"
                            : "bg-background border-border text-gray-400 hover:text-red-400"
                        }`}
                      >
                        <Ban className="w-3.5 h-3.5" />
                        {clienteActual.es_spam ? "Quitar Spam" : "Spam"}
                      </button>
                      <button
                        onClick={toggleAgenteIA}
                        className={`flex-1 flex justify-center items-center gap-1.5 py-2 rounded-lg border text-xs font-medium transition-all ${
                      ...
                      >
                        <Bot className="w-3.5 h-3.5" />
                        {selectedConv.agente_activo ? "IA Activa" : "IA Pausa"}
                      </button>
                    </div>
                    ...
                    <div className="bg-background p-4 rounded-xl border border-border space-y-3">
                      ...
                      <div className="space-y-1.5">
                        {tareasCliente.map((t) => (
                          <div key={t.id} className="flex items-start gap-2 bg-surface p-2 rounded-lg border border-border group">
                            <button onClick={() => toggleTarea(t.id, t.completada)} className="mt-0.5 flex-shrink-0 text-purple-400">
                              {t.completada ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4 text-gray-500" />}
                            </button>
                            <div className="flex-1 min-w-0">
                              <p className={`text-xs ${t.completada ? "line-through text-gray-500" : "text-gray-200"}`}>
                                {t.titulo}
                              </p>
                              {t.fecha_vencimiento && (
                                <p className="text-[9px] text-amber-400/80 mt-0.5">{t.fecha_vencimiento}</p>
                              )}
                            </div>
                            <button
                              onClick={() => eliminarTarea(t.id)}
                              className="text-gray-600 hover:text-red-400 md:opacity-0 group-hover:opacity-100 transition-opacity"
                            >
                        ...
                        {tareasCliente.length === 0 && (
                          <p className="text-[11px] text-gray-600 italic">Sin tareas</p>
                        )}
                      </div>
                      <form onSubmit={agregarTarea} className="pt-2 border-t border-border/50 flex flex-col gap-2">
                        <input
                          ...
                          className="w-full bg-surface border border-border rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-purple-500"
                          required
                        />
                        <div className="flex gap-2">
                          <input
                            ...
                            className="flex-1 bg-surface border border-border rounded-lg px-2 py-1.5 text-xs text-gray-400 focus:outline-none"
                          />
                          <button
                            type="submit"
                            className="bg-surface border border-border hover:bg-purple-600 hover:text-white px-3 rounded-lg transition-colors"
                          >
                    ...
                    <div className="bg-background p-4 rounded-xl border border-border space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-emerald-400 font-bold border border-emerald-900/50 bg-emerald-950/30 px-1.5 py-0.5 rounded">
                          $
                          {pagosCliente
                            .reduce((acc, p) => acc + (p.estado === "pagado" ? Number(p.monto) : 0), 0)
                            .toLocaleString()}{" "}
                          / $
                          {pagosCliente.reduce((acc, p) => acc + Number(p.monto), 0).toLocaleString()}
                        </span>
                      </div>
                      ...
                      {pagosCliente.length > 0 && (
                        <div className="space-y-1.5">
                          {pagosCliente.map((pago) => (
                            ...
                            >
                              <button onClick={() => marcarPago(pago.id, pago.estado)} className="mr-2 flex-shrink-0">
                                {pago.estado === "pagado" ? (
                              ...
                              <div className="flex-1 min-w-0">
                                <span className={pago.estado === "pagado" ? "line-through text-gray-500" : "text-gray-200"}>
                                  ${Number(pago.monto).toLocaleString()}
                                </span>
                                <div className="text-[9px] text-gray-500 truncate">
                                  {pago.notas} • {pago.fecha_vencimiento}
                                </div>
                              </div>
                              <button onClick={() => eliminarPago(pago.id)} className="text-gray-600 hover:text-red-400 ml-1">
                                <Trash2 className="w-3.5 h-3.5" />
                              </button>
                            </div>
                          ))}
                      ...
                      <form onSubmit={agregarPagos} className="pt-2 border-t border-border/50 flex flex-col gap-2">
                        <div className="flex bg-surface p-0.5 rounded-lg text-[10px]">
                          <button
                            ...
                            className={`flex-1 py-1.5 rounded-md transition-all ${
                              tipoPago === "unico" ? "bg-purple-600 text-white" : "text-gray-400"
                            }`}
                          >
                            Único
                          </button>
                          <button
                            ...
                            className={`flex-1 py-1.5 rounded-md transition-all ${
                              tipoPago === "cuotas" ? "bg-purple-600 text-white" : "text-gray-400"
                            }`}
                          >
                            Cuotas
                          </button>
                        ...
                        <input
                          ...
                          className="w-full bg-surface border border-border rounded-lg px-2.5 py-1.5 text-xs focus:outline-none focus:border-purple-500"
                          required
                        />
                        {tipoPago === "cuotas" && (
                          <div className="flex items-center gap-2 text-xs text-gray-400">
                            ...
                            <input
                              ...
                              className="w-14 bg-surface border border-border rounded-lg px-2 py-1 text-center text-xs focus:outline-none"
                            />
                            <span>cuotas</span>
                          </div>
                        )}
                        <div className="flex gap-2">
                          ...
                          <select
                            ...
                            className="flex-1 bg-surface border border-border rounded-lg px-2 py-1.5 text-xs text-gray-300 focus:outline-none"
                          >
                        ...
                        <input
                          type="text"
                          placeholder="Nota: amarre, limpieza..."
                          value={notaPago}
                        ...
                        <button
                          type="submit"
                          className="w-full bg-surface border border-border hover:bg-purple-600 text-gray-300 hover:text-white py-1.5 rounded-lg text-xs transition-colors flex items-center justify-center gap-1"
                        >
          ...
          <div className="flex-1 flex flex-col h-full overflow-hidden bg-background">
            <header className="p-4 md:p-6 border-b border-border flex items-center justify-between bg-surface/30">
              ...
              >
                ...
                {isEditingPipeline ? "Cerrar" : "Configurar"}
              </button>
            </header>
            ...
            <div className="flex-1 flex overflow-x-auto p-4 md:p-6 gap-4">
              {isEditingPipeline && (
                <div className="w-72 flex-shrink-0 bg-surface border border-border rounded-2xl p-4 flex flex-col gap-4 shadow-xl">
                  <h2 className="text-sm font-bold text-purple-300 flex items-center gap-2 border-b border-border pb-2">
                    <Edit2 className="w-4 h-4" /> Editar Etapas
                  </h2>
                    ...
                    {pipelineEtapas.map((etapa, idx) => (
                      <div key={etapa.id} className="flex items-center gap-2 bg-background p-2 rounded-lg border border-border">
                        <div className="flex flex-col gap-1">
                          <button onClick={() => moverEtapa(idx, -1)} className="text-gray-500 hover:text-white">
                        ...
                        />
                        <button onClick={() => eliminarEtapa(etapa.id)} className="text-red-500/70 hover:text-red-400 p-1">
                          <Trash2 className="w-4 h-4" />
                        </button>
                      </div>
                    ))}
                  ...
                  <button
                    onClick={agregarEtapaPipeline}
                    className="w-full py-2 bg-surfaceHover border border-dashed border-gray-600 rounded-lg text-xs text-gray-400 hover:text-white flex items-center justify-center gap-2"
                  >
              ...
              {pipelineEtapas
                .filter((e) => e.clave !== "spam")
                .map((col) => {
                  const clientesEnCol = conversaciones.filter(
                    (c) => (c.clientes?.estado || "nuevo_lead") === col.clave && !c.clientes?.es_spam
                  );
                  return (
                    <div
                      key={col.id}
                      className="w-72 flex-shrink-0 bg-surface/60 border border-border rounded-2xl p-4 flex flex-col gap-3 min-h-full"
                    >
          ...
          <div className="flex-1 p-4 md:p-8 overflow-y-auto bg-background">
            <header className="mb-6">
              <h1 className="text-xl md:text-2xl font-bold text-gray-100 flex items-center gap-2">
                <ListTodo className="text-purple-400 w-6 h-6" /> Panel de Tareas
              </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 items-start">
              ...
              <div className="bg-surface/50 border border-border rounded-2xl p-4 md:p-5 flex flex-col gap-4">
                ...
                <div className="space-y-2">
                  {todasTareas.filter(
                    (t) => !t.completada && t.fecha_vencimiento && new Date(t.fecha_vencimiento) <= ahora
                  ).length === 0 && <p className="text-xs text-gray-500 italic">Todo al día ✅</p>}
                  {todasTareas
                    .filter((t) => !t.completada && t.fecha_vencimiento && new Date(t.fecha_vencimiento) <= ahora)
                    .map((t) => (
                      <div key={t.id} className="bg-background border border-amber-900/30 p-3 rounded-xl">
                        <div className="flex justify-between items-start mb-1">
                          <span className="text-[10px] bg-amber-950/50 text-amber-500 px-1.5 rounded border border-amber-900">
                            {t.fecha_vencimiento}
                          </span>
                          <span className="text-[10px] text-gray-400 truncate max-w-[120px]">
                            {t.clientes?.nombre || "Cliente"}
                          </span>
                        </div>
                        <p className="text-xs text-gray-200 font-medium">{t.titulo}</p>
                        <button
                          onClick={() => toggleTarea(t.id, false)}
              ...
              <div className="bg-surface/50 border border-border rounded-2xl p-4 md:p-5 flex flex-col gap-4">
                ...
                <div className="space-y-2">
                  {todasTareas
                    .filter(
                      (t) =>
                        !t.completada && (!t.fecha_vencimiento || new Date(t.fecha_vencimiento) > ahora)
                    )
                    .map((t) => (
      ...
      {showAdmin && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm bg-surface border border-border rounded-2xl p-6 space-y-4 shadow-2xl">
            ...
            {balances?.error && (
              <div className="text-xs text-red-400 bg-red-950/30 border border-red-800/40 p-3 rounded-lg">
                {balances.error}
              </div>
            )}
              ...
              <div className="space-y-3 text-xs">
                <div className="p-3 rounded-xl bg-background border border-border">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className={`w-2 h-2 rounded-full ${balances.openai?.ok ? "bg-emerald-500" : "bg-red-500"}`} />
                  </div>
                  <div className="text-xl font-bold text-gray-100">
                    {balances.openai?.balance != null ? `$${Number(balances.openai.balance).toFixed(2)}` : "—"}
                  </div>
                ...
                <div className="p-3 rounded-xl bg-background border border-border">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-gray-400 font-medium">Fish Audio</span>
                    <span className={`w-2 h-2 rounded-full ${balances.fish?.ok ? "bg-emerald-500" : "bg-red-500"}`} />
                  </div>
                  <div className="text-xl font-bold text-gray-100">
                    {balances.fish?.balance != null ? `$${Number(balances.fish.balance).toFixed(2)}` : "—"}
                  </div>
                  <div className="text-[10px] text-gray-500 mt-1">{balances.fish?.note}</div>
                </div>

                <div className="text-[10px] text-gray-500 text-center pt-1">
                  {balances.updatedAt ? new Date(balances.updatedAt).toLocaleString() : ""}
                </div>
              </div>
            )}
          </div>
